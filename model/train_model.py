...
artifacts = {
    ...
    "mae": mae,
    "weights": {
        "w1": float(importances[0]),
        "w2": float(importances[1]),
        "w3": float(importances[2])
    }
}

OUTPUT_FILE = os.path.join(os.path.dirname(__file__), "model_artifacts.json")
with open(OUTPUT_FILE, 'w') as f:
    json.dump(artifacts, f, indent=2)

print(f"Model artifacts saved to {OUTPUT_FILE}")
